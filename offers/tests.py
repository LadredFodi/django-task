...
from offers.tasks import _execute_purchase, _find_matching_dealerships, process_customer_offer
...
class TestOfferModel:
    ...
    def test_offer_status_choices(self, offer):
        ...
        for offer_status in valid_statuses:
...
class TestOfferService:
    ...
    def test_create_offer(self, customer, car_model, dealership_inventory):
        offer = OfferService.create_offer(customer=customer, car_model_id=car_model.id, max_price=32000.00)
...
class TestOfferCeleryTasks:
    ...
    def test_process_customer_offer_success(self, offer, dealership_inventory, customer):
        dealership_inventory.selling_price = Decimal("30000.00")
    ...
    def test_process_customer_offer_no_matching_dealerships(self, offer, dealership_inventory):
    ...
    def test_process_customer_offer_insufficient_balance(self, offer, dealership_inventory, customer):
        dealership_inventory.selling_price = Decimal("30000.00")
        dealership_inventory.save()
    ...
    def test_find_matching_dealerships_with_promotion(self, offer, dealership_inventory, promotion_dealership):
    ...
    def test_execute_purchase_insufficient_inventory(self, offer, dealership_inventory, customer):
...
class TestOfferAPI:
    ...
    def test_create_offer(self, authenticated_client, customer, car_model):
        url = reverse("offer-list")
        data = {"car_model": car_model.id, "max_price": "33000.00"}

        response = authenticated_client.post(url, data, format="json")

        assert response.status_code == status.HTTP_201_CREATED
        assert Offer.objects.filter(customer=customer, car_model=car_model).exists()

    def test_update_offer(self, authenticated_client, offer):
        url = reverse("offer-detail", kwargs={"pk": offer.pk})
        data = {"max_price": "37000.00", "notes": "Updated notes"}
    ...
    def test_filter_offers_by_status(self, authenticated_client, customer, car_model):
        Offer.objects.create(customer=customer, car_model=car_model, max_price=Decimal("30000.00"), status="pending")
        Offer.objects.create(customer=customer, car_model=car_model, max_price=Decimal("35000.00"), status="completed")
    ...
    def test_search_offers(self, authenticated_client, offer):
        url = reverse("offer-list")
        response = authenticated_client.get(url, {"search": offer.customer.user.username})
    ...
    def test_cancel_offer_endpoint(self, authenticated_client, regular_user, customer, car_model, db):
        ...
        offer = Offer.objects.create(
            customer=customer, car_model=car_model, max_price=Decimal("35000.00"), status="pending"
        )
