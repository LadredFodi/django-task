...
from offers.serializers import OfferCreateSerializer, OfferListSerializer, OfferSerializer
...
class OfferViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ["customer__user__username", "car_model__brand", "car_model__model", "matched_dealership__name"]
    ...
    def process(self, request, pk=None):
        ...
        if offer.status != "pending":
            return Response({"error": "Offer already processed or being processed"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def complete(self, request, pk=None):
        ...
        if offer.status == "completed":
            return Response({"error": "Offer already completed"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def cancel(self, request, pk=None):
        ...
        if not request.user.is_staff:
            if offer.customer.user != request.user:
                return Response(
                    {"error": "You do not have permission to cancel this offer"}, status=status.HTTP_403_FORBIDDEN
                )
            if offer.status != "pending":
                return Response({"error": "You can only cancel pending offers"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def search_results_detail(self, request, pk=None):
        ...
        if not offer.search_results:
            return Response({"message": "Search results are not available yet"}, status=status.HTTP_404_NOT_FOUND)
