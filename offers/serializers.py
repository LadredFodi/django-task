...
class OfferSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            'id', 'customer', 'status', 'matched_dealership', 'matched_price',
            'processed_at', 'completed_at', 'search_results', 'created_at', 'updated_at'
        ]
    ...
    def validate(self, data):
        ...
        if max_price and customer.balance < max_price:
            raise serializers.ValidationError(
                f"Insufficient funds on balance. Available: ${customer.balance}"
            )
    ...
    def validate(self, data):
        ...
        if request and hasattr(request.user, 'customer_profile'):
            ...
            if not customer.email_verified:
                raise serializers.ValidationError(
                    "To create an offer, you need to confirm your email."
                )
