...
class OfferService:
    ...
    def get_all_active_offers() -> QuerySet[Offer]:
        return Offer.objects.select_related("customer", "car_model", "matched_dealership").filter(is_active=True)
    ...
    def get_offers_by_customer(customer: Customer) -> QuerySet[Offer]:
        return (
            Offer.objects.filter(customer=customer, is_active=True)
            .select_related("car_model", "matched_dealership")
            .order_by("-created_at")
        )

    @staticmethod
    def get_pending_offers() -> QuerySet[Offer]:
        return Offer.objects.filter(status="pending", is_active=True).select_related("customer", "car_model")

    @staticmethod
    def create_offer(customer: Customer, car_model_id: int, max_price: float, **kwargs: Any) -> Offer:
        offer = Offer.objects.create(customer=customer, car_model_id=car_model_id, max_price=max_price, **kwargs)
    ...
    def search_matching_dealerships(offer: Offer) -> List[Dict[str, Any]]:
        ...
        if results:
            offer.matched_dealership_id = int(results[0].get("dealership_id", 0))  # type: ignore
            offer.matched_price = Decimal(str(results[0].get("price", 0)))  # type: ignore
    ...
    def expire_old_offers() -> int:
        ...
        expired_count = Offer.objects.filter(
            status__in=["pending", "processing"], expires_at__lt=now, is_active=True
    ...
    def get_statistics() -> dict:
        ...
        top_models = list(
            queryset.values("car_model__brand", "car_model__model").annotate(count=Count("id")).order_by("-count")[:10]
        )
