...
def regular_user(db):
    return User.objects.create_user(
        username="user", email="user@test.com", password="userpass123", first_name="John", last_name="Doe"
    )
...
def supplier_discount(db, supplier, dealership):
    return SupplierDiscount.objects.create(
        supplier=supplier, dealership=dealership, discount_percent=Decimal("5.00"), min_purchases=10, is_applied=False
    )
...
def promotion_supplier(db, promotion, supplier):
    return PromotionSupplier.objects.create(promotion=promotion, supplier=supplier, discount_percent=Decimal("10.00"))
...
def offer(db, customer, car_model):
    return Offer.objects.create(customer=customer, car_model=car_model, max_price=Decimal("35000.00"), status="pending")
