...
from dealerships.models import Dealership, DealershipInventory, Purchase, DealershipPreference
...
def _analyze_demand(dealership: Dealership) -> Dict[int, int]:
    ...
    sales_stats = dealership.sales.filter(
        is_active=True,
        created_at__gte=thirty_days_ago
    ).values('car_model_id').annotate(
        sold_count=Count('id')
    ).order_by('-sold_count')
...
def _get_models_to_purchase(
    dealership: Dealership,
    preferences: Optional[DealershipPreference],
    demand_analysis: Dict[int, int]
) -> Dict[int, int]:
    """
