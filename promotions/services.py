...
class PromotionService:
    ...
    def get_promotion_by_id(promotion_id: int) -> Optional[Promotion]:
        try:
            return Promotion.objects.prefetch_related("car_models").get(id=promotion_id, is_active=True)
    ...
    def get_active_now_promotions() -> QuerySet[Promotion]:
        now = timezone.now()
        return Promotion.objects.filter(is_active=True, start_date__lte=now, end_date__gte=now).prefetch_related(
            "car_models"
        )
    ...
    def get_upcoming_promotions() -> QuerySet[Promotion]:
        now = timezone.now()
        return (
            Promotion.objects.filter(is_active=True, start_date__gt=now)
    ...
    def check_applicable(
        promotion: Promotion, car_model_id: Optional[int] = None, amount: Optional[Decimal] = None
    ) -> bool:
...
class PromotionDealershipService:
    ...
    def get_all_active_dealership_promotions() -> QuerySet[PromotionDealership]:
        return PromotionDealership.objects.select_related("promotion", "dealership").filter(is_active=True)

    @staticmethod
    def get_dealership_promotions_by_dealership(dealership_id: int) -> QuerySet[PromotionDealership]:
        return PromotionDealership.objects.filter(dealership_id=dealership_id, is_active=True).select_related(
            "promotion"
        )
    ...
    def check_exists(promotion_id: int, dealership_id: int) -> bool:
        return PromotionDealership.objects.filter(
            promotion_id=promotion_id, dealership_id=dealership_id, is_active=True
        ).exists()
    ...
    def get_top_dealerships(limit: int = 10):
        return (
            PromotionDealership.objects.filter(times_applied__gt=0, is_active=True)
            .select_related("dealership", "promotion")
            .order_by("-times_applied")[:limit]
        )
...
class PromotionSupplierService:
    ...
    def get_all_active_supplier_promotions() -> QuerySet[PromotionSupplier]:
        return PromotionSupplier.objects.select_related("promotion", "supplier").filter(is_active=True)

    @staticmethod
    def get_supplier_promotions_by_supplier(supplier_id: int) -> QuerySet[PromotionSupplier]:
        return PromotionSupplier.objects.filter(supplier_id=supplier_id, is_active=True).select_related("promotion")

    @staticmethod
    def check_exists(promotion_id: int, supplier_id: int) -> bool:
        return PromotionSupplier.objects.filter(
            promotion_id=promotion_id, supplier_id=supplier_id, is_active=True
        ).exists()
    ...
    def get_top_suppliers(limit: int = 10) -> QuerySet[PromotionSupplier]:
        return (
            PromotionSupplier.objects.filter(times_applied__gt=0, is_active=True)
            .select_related("supplier", "promotion")
            .order_by("-times_applied")[:limit]
        )
