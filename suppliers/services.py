...
class SupplierService:
    ...
    def get_supplier_statistics(supplier: Supplier) -> dict[str, Any]:
        ...
        purchases_stats = Purchase.objects.filter(supplier=supplier, is_active=True).aggregate(
        ...
        top_partner_dealerships = list(
            ...
            .annotate(orders_count=Count("id"), total_quantity=Sum("quantity"), total_revenue=Sum("total_price"))
        ...
        top_selling_models = list(
            ...
            .annotate(orders_count=Count("id"), total_quantity=Sum("quantity"))
...
class SupplierCarService:
    ...
    def get_supplier_cars_by_supplier(supplier_id: int) -> QuerySet[SupplierCar]:
        return SupplierCar.objects.filter(supplier_id=supplier_id, is_active=True).select_related("car_model")
    ...
    def get_best_prices() -> QuerySet[SupplierCar]:
        best_prices = SupplierCar.objects.filter(is_active=True).values("car_model").annotate(min_price=Min("price"))

        car_ids = []
        for item in best_prices:
            car = SupplierCar.objects.filter(
                car_model=item["car_model"], price=item["min_price"], is_active=True
            ).first()
            if car:
...
class SupplierDiscountService:
    ...
    def get_all_active_supplier_discounts() -> QuerySet[SupplierDiscount]:
        return SupplierDiscount.objects.select_related("supplier", "dealership").filter(is_active=True)
    ...
    def check_and_apply_discount(supplier_id: int, dealership_id: int) -> Optional[SupplierDiscount]:
        try:
            discount = SupplierDiscount.objects.get(
                supplier_id=supplier_id, dealership_id=dealership_id, is_active=True
            )
